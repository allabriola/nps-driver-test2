...
// ── Resumo Executivo Driver (estilo buildStrategicSummary) ──────────────────
function buildDDResumoExec(drv, period, d, bkData) {{
  var isMes = period === 'mes';
  var isVig = period === 'vig';
  var lA = isMes ? M2_LABEL : (isVig ? S1_LABEL : S2_LABEL);
  var lB = isMes ? M1_LABEL : (isVig ? VIG_LABEL : S1_LABEL);
  var tgt = d ? d.target : null;
  var cat = d ? d.cat : '';

  // NPS dos pontos históricos
  var pts = isMes ? d.monthly : (isVig ? ((DRV_HIST[drv]||{{}}).weekly_vig || d.weekly) : d.weekly);
  var cur  = pts && pts.length > 0 ? pts[pts.length-1] : null;
  var prev = pts && pts.length > 1 ? pts[pts.length-2] : null;
  var npsB = cur  ? cur.nps  : null;
  var npsA = prev ? prev.nps : null;
  var surv = cur  ? (cur.s || 0) : 0;
  ...
  // Cor do header: vermelho (>1pp abaixo), laranja (0-1pp abaixo), verde (acima)
  var hColor = (gapTgt!==null && gapTgt < -1) ? '#b71c1c' :
               (gapTgt!==null && gapTgt <  0) ? '#e65100' : '#1b5e20';
  var statusIcon  = (gapTgt!==null && gapTgt < 0) ? '&#9888;&#65039;' : '&#9989;';
  var statusLabel = gapTgt===null ? '&mdash;' :
    gapTgt >= 0 ? '&#10004; Acima da meta (+' + gapTgt.toFixed(2) + 'pp)' :
    '&#9888; Abaixo da meta (' + gapTgt.toFixed(2) + 'pp)';
  var lPeriod = isMes ? 'MoM' : isVig ? 'VIG &#9889;' : 'WoW';

  function nS(v) {{ return v!==null&&v!==undefined ? v.toFixed(1)+'%' : '&mdash;'; }}
  function dS(v) {{ return v!==null ? (v>=0?'+':'')+v.toFixed(2)+'pp' : '&mdash;'; }}
  function clr(v) {{ return v>=0 ? '#1b5e20' : '#b71c1c'; }}

  // Impactos por processo via bkData
  var pA = isMes ? 'M2' : (isVig ? 'S1' : 'S2');
  var pB = isMes ? 'M1' : (isVig ? ((bkData&&bkData.P&&bkData.P.VIG)?'VIG':'S1') : 'S1');
  var procs = [];
  if (bkData && bkData.P) {{
    var dA = bkData.P[pA]||{{}}, dB = bkData.P[pB]||{{}};
    var keys = Object.keys(Object.assign({{}}, dA, dB));
    var totSA=0,totSB=0,totPB=0,totDB=0;
    keys.forEach(function(k){{
      var a=dA[k]||{{s:0}}; var b=dB[k]||{{s:0,p:0,d:0}};
      totSA+=a.s||0; totSB+=b.s||0; totPB+=b.p||0; totDB+=b.d||0;
    }});
    var npsAll = totSB>0 ? (totPB-totDB)/totSB*100 : null;
    procs = keys.map(function(k){{
      var a=dA[k]||{{p:0,d:0,s:0,nps:null}}; var b=dB[k]||{{p:0,d:0,s:0,nps:null}};
      var shaA=totSA>0?(a.s||0)/totSA:0, shaB=totSB>0?(b.s||0)/totSB:0;
      var neto=(shaA>0&&a.nps!==null&&b.nps!==null)?shaA*(b.nps-a.nps):0;
      var mix =(b.nps!==null&&npsAll!==null)?(shaB-shaA)*(b.nps-npsAll):0;
      return {{k:k, nA:a.nps, nB:b.nps,
        delta:(a.nps!==null&&b.nps!==null?Math.round((b.nps-a.nps)*100)/100:null),
        impact:Math.round((neto+mix)*100)/100,
        gapT:tgt!==null&&b.nps!==null?Math.round((b.nps-tgt)*100)/100:null,
        shaB:Math.round(shaB*1000)/10, sB:b.s||0}};
    }}).filter(function(x){{return x.sB>0;}}).sort(function(a,b){{return a.impact-b.impact;}});
  }}
  ...
  // Bullets qualitativos do DD_SUMMARIES
  var sumObj=(typeof DD_SUMMARIES!=='undefined'?DD_SUMMARIES:{{}})[drv]||{{}};
  var sumRaw=sumObj[isMes?'mom':'wow']||sumObj.mom||null;
  var sumText=(typeof sumRaw==='object'&&sumRaw)?(sumRaw.bullets_legado||''):(sumRaw||'');
  var bullets=sumText.split('\\n').map(function(b){{return b.replace(/^[\\s\\u25B6\\u2022]+/,'').trim();}})
    .filter(function(b){{return b.length>20;}}).slice(0,3);

  // ── HTML ──
  var html =
    '<div style="margin:12px 0 18px;border:2px solid '+hColor+';border-radius:12px;overflow:hidden">'+
    '<div style="background:'+hColor+';padding:10px 16px;display:flex;align-items:center;'+
         'justify-content:space-between;flex-wrap:wrap;gap:6px">'+
      '<div style="display:flex;align-items:center;gap:8px">'+
        '<span style="font-size:15px">'+statusIcon+'</span>'+
        '<span style="color:#fff;font-weight:700;font-size:13px">Resumo Executivo &mdash; '+drv+'</span>'+
        (cat?'<span style="color:rgba(255,255,255,.72);font-size:11px">&middot; '+cat+'</span>':'')+
      '</div>'+
      '<div style="color:#fff;font-size:11px;font-weight:600">'+statusLabel+'</div>'+
    '</div>'+
    '<div style="padding:12px 16px;background:#fafafa">';
  ...
  html +=
    '<div style="display:flex;gap:10px;margin-bottom:12px;flex-wrap:wrap">'+
      '<div style="background:#fff;padding:7px 14px;border-radius:8px;border:1px solid #e0e0e0;min-width:105px">'+
        '<div style="font-size:10px;color:#888;font-weight:600">NPS '+lB+'</div>'+
        '<div style="font-size:20px;font-weight:700;color:'+(gapTgt!==null?clr(gapTgt):'#333')+'">'+nS(npsB)+'</div>'+
        '<div style="font-size:11px;color:#666">'+dS(delta)+' vs '+lA+'</div>'+
      '</div>'+
      '<div style="background:#fff;padding:7px 14px;border-radius:8px;border:1px solid #e0e0e0;min-width:105px">'+
        '<div style="font-size:10px;color:#888;font-weight:600">Target</div>'+
        '<div style="font-size:20px;font-weight:700;color:#333">'+(tgt!==null?tgt.toFixed(1)+'%':'&mdash;')+'</div>'+
        '<div style="font-size:11px;color:'+(gapTgt!==null?clr(gapTgt):'#666')+'">'+dS(gapTgt)+' gap</div>'+
      '</div>'+
      (surv>0?
        '<div style="background:#fff;padding:7px 14px;border-radius:8px;border:1px solid #e0e0e0;min-width:105px">'+
          '<div style="font-size:10px;color:#888;font-weight:600">Pesquisas</div>'+
          '<div style="font-size:20px;font-weight:700;color:#333">'+surv.toLocaleString('pt-BR')+'</div>'+
          '<div style="font-size:11px;color:#666">'+lB+'</div>'+
        '</div>':'')+
    '</div>';

  // Processos negativos e positivos (grid 2 colunas)
  if (top3neg.length > 0 || top2pos.length > 0) {{
    html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">';
    if (top3neg.length > 0) {{
      html +=
        '<div style="background:#fff;border-radius:9px;padding:10px 12px;border:1px solid #ef9a9a">'+
        '<div style="font-size:10px;font-weight:700;color:#b71c1c;margin-bottom:7px">&#128201; PROCESSOS EM QUEDA</div>';
      top3neg.forEach(function(p) {{
        html +=
          '<div style="font-size:11px;color:#333;margin-bottom:5px;padding:5px 9px;'+
              'background:#fff5f5;border-radius:6px;border-left:3px solid #ef9a9a">'+
            '<b>'+p.k+'</b>'+
            '<div style="color:#666;font-size:10px;margin-top:2px">'+
              'NPS '+nS(p.nB)+'&nbsp;&middot;&nbsp;'+
              '<span style="color:#b71c1c;font-weight:600">'+dS(p.impact)+' impacto</span>'+
    ...
    if (top2pos.length > 0) {{
      html +=
        '<div style="background:#fff;border-radius:9px;padding:10px 12px;border:1px solid #a5d6a7">'+
        '<div style="font-size:10px;font-weight:700;color:#1b5e20;margin-bottom:7px">&#128200; PROCESSOS POSITIVOS</div>';
      top2pos.forEach(function(p) {{
        html +=
          '<div style="font-size:11px;color:#333;margin-bottom:5px;padding:5px 9px;'+
              'background:#f1f8f3;border-radius:6px;border-left:3px solid #a5d6a7">'+
            '<b>'+p.k+'</b>'+
            '<div style="color:#666;font-size:10px;margin-top:2px">'+
              'NPS '+nS(p.nB)+'&nbsp;&middot;&nbsp;'+
              '<span style="color:#1b5e20;font-weight:600">'+dS(p.impact)+' impacto</span>'+
            '</div>'+
          '</div>';
      }});
      html += '</div>';
  ...
  if (bullets.length > 0) {{
    html +=
      '<div style="background:#fff;border-radius:9px;padding:10px 12px;border:1px solid #dde2f0">'+
      '<div style="font-size:10px;font-weight:700;color:#3949ab;margin-bottom:7px">&#128221; INSIGHTS DO PER&Iacute;ODO</div>';
    bullets.forEach(function(b) {{
      html +=
        '<div style="font-size:11px;color:#2d3561;margin-bottom:5px;padding:5px 9px;'+
            'background:#f5f6ff;border-radius:6px;border-left:3px solid #3949ab">'+
          '&#9654; '+b+
        '</div>';
    }});
    html += '</div>';
...
function renderDD(period) {{
  ...
  if (period === 'mes') {{
    ...
    content.innerHTML =
      ...
      '</div>' +
      buildDDResumoExec(drv,'mes',d,bkMesDD) +
    ...
    content.innerHTML =
      '<div class="dd-sc-grid">' +
        ...
        sc_dd('Gap vs Target', fmtDelta(gapTgt), gapTgt, null, null) +
      '</div>' +
      buildDDResumoExec(drv,period,d,bkSemDD) +
      '<div class="dd-chart-section">' +
