...
.bk-table{{width:100%;border-collapse:collapse;background:#fff;border-radius:8px;
          overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.06);margin-bottom:12px;
          font-size:12px}}
.bk-table thead tr{{background:#2d3277}}
.bk-table thead th{{color:#fff;padding:8px 10px;font-size:10px;font-weight:700;
                   text-transform:uppercase;letter-spacing:.4px;white-space:nowrap}}
.bk-table thead th.col-name{{text-align:left;min-width:160px}}
.bk-table thead th:not(.col-name){{text-align:right;min-width:58px}}
.bk-table thead th.col-impact{{background:#1a1e3c;min-width:72px}}
.bk-table tbody td{{padding:6px 10px;border-bottom:1px solid #f2f2f5;vertical-align:middle}}
.bk-table tbody tr:last-child td{{border-bottom:none}}
.bk-table tbody tr:hover td{{background:#f7f8ff}}
.bk-table tbody tr.bk-total td{{background:#f0f2f8;font-weight:700;
                                border-top:2px solid #dde0ea}}
.bk-name{{font-weight:500;text-align:left}}
.bk-surv{{color:#bbb!important;font-size:11px!important;text-align:right}}
.bk-impact-cell{{font-weight:700;text-align:right;white-space:nowrap}}
.bk-impact-pos-hi{{background:#e8f5e9!important;color:#1b5e20!important}}
.bk-impact-pos-lo{{background:#f1f8e9!important;color:#33691e!important}}
.bk-impact-neg-hi{{background:#ffebee!important;color:#b71c1c!important}}
.bk-impact-neg-lo{{background:#fff3f3!important;color:#c62828!important}}
.bk-impact-neu{{color:#999!important}}
</style>
</head>
<body>
...
function buildBreakdownTable(drvData, dim, periodA, periodB) {{
    ...
    // Totais do driver nos dois periodos (base para share e NPS consolidado)
    ...
    var lA = periodA==='M2'?M2_LABEL:(periodA==='M1'?M1_LABEL:(periodA==='S2'?S2_LABEL:S1_LABEL));
    var lB = periodB==='M1'?M1_LABEL:(periodB==='S1'?S1_LABEL:(periodB==='M2'?M2_LABEL:S2_LABEL));

    function fNPS(v)  {{ return v!==null ? v.toFixed(1)+'%' : '&mdash;'; }}
    function fDelta(v){{ return v===null ? '&mdash;' : (v>=0?'+':'')+v.toFixed(2)+' pp'; }}
    function dCls(v)  {{ return v===null?'':(v>=0?'pos':'neg'); }}
    function iCls(v)  {{
        if(v===null) return 'bk-impact-neu';
        if(v>= 0.30) return 'bk-impact-pos-hi';
        if(v>= 0.05) return 'bk-impact-pos-lo';
        if(v<=-0.30) return 'bk-impact-neg-hi';
        if(v<=-0.05) return 'bk-impact-neg-lo';
        return 'bk-impact-neu';
    }}

    var html = '<table class="bk-table"><thead><tr>' +
    ...
    var totImpact=0, totNeto=0, totMix=0;
    items.forEach(function(item) {{
        var a=item.a, b=item.b;
        var delta = (a.nps!==null && b.nps!==null) ? b.nps-a.nps : null;
        totImpact += item.impact; totNeto += item.neto; totMix += item.mix;
        html += '<tr>' +
            '<td class="bk-name">' + item.k + '</td>' +
            '<td class="num">' + fNPS(a.nps) + '</td>' +
            '<td class="num">' + fNPS(b.nps) + '</td>' +
            '<td class="num ' + dCls(delta) + '">' + fDelta(delta) + '</td>' +
            '<td class="num bk-surv">' + (b.s||0).toLocaleString('pt-BR') + '</td>' +
            '<td class="num bk-surv">' + (item.shaB*100).toFixed(1)+'%</td>' +
            '<td class="num ' + dCls(item.neto) + '">' + fDelta(item.neto) + '</td>' +
            '<td class="num ' + dCls(item.mix)  + '">' + fDelta(item.mix)  + '</td>' +
            '<td class="num bk-impact-cell ' + iCls(item.impact) + '">' + fDelta(item.impact) + '</td>' +
            '</tr>';
    }});

    totImpact=Math.round(totImpact*100)/100;
    totNeto  =Math.round(totNeto*100)/100;
    totMix   =Math.round(totMix*100)/100;
    var totDelta = (npsA_tot!==null && npsB_tot!==null) ? npsB_tot-npsA_tot : null;

    html += '<tr class="bk-total">' +
        '<td class="bk-name">Total</td>' +
        '<td class="num">' + fNPS(npsA_tot) + '</td>' +
        '<td class="num">' + fNPS(npsB_tot) + '</td>' +
        '<td class="num ' + dCls(totDelta) + '">' + fDelta(totDelta!==null?Math.round(totDelta*100)/100:null) + '</td>' +
        '<td class="num bk-surv">' + totB.s.toLocaleString('pt-BR') + '</td>' +
        '<td class="num bk-surv">100%</td>' +
        '<td class="num ' + dCls(totNeto) + '">' + fDelta(totNeto) + '</td>' +
        '<td class="num ' + dCls(totMix)  + '">' + fDelta(totMix)  + '</td>' +
        '<td class="num bk-impact-cell ' + iCls(totImpact) + '">' + fDelta(totImpact) + '</td>' +
        '</tr>';
