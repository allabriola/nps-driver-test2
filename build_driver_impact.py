...
.bk-table thead th.col-name{{text-align:left}}
.bk-table thead th:not(.col-name){{text-align:right}}
.bk-table thead th.col-impact{{background:#1a1e3c}}
.bk-table tbody td{{padding:6px 10px;border-bottom:1px solid #f2f2f5;
                   vertical-align:middle;overflow:hidden;text-overflow:ellipsis;
                   white-space:nowrap}}
.bk-table tbody td.col-name{{white-space:normal;word-break:break-word}}
.bk-table tbody td:not(.col-name){{text-align:right;
...
.bk-impact-cell{{font-weight:700}}
...
function buildBreakdownTable(drvData, dim, periodA, periodB) {{
    ...
    var html = '<table class="bk-table">' +
        ...
        '<col style="width:8%">'  +  // NPS A
        '<col style="width:8%">'  +  // NPS B
        '<col style="width:9%">'  +  // delta
        '<col style="width:9%">'  +  // surveys
        '<col style="width:7%">'  +  // share
        '<col style="width:9%">'  +  // neto
        '<col style="width:9%">'  +  // mix
        '<col style="width:11%">' +  // impacto
        '</colgroup>' +
        '<thead><tr>' +
    ...
    items.forEach(function(item) {{
        ...
        html += '<tr>' +
            '<td class="bk-name col-name">' + item.k + '</td>' +
            '<td>' + fNPS(a.nps) + '</td>' +
            '<td>' + fNPS(b.nps) + '</td>' +
            '<td class="' + dCls(delta) + '">' + fDelta(delta) + '</td>' +
            '<td class="bk-surv">' + (b.s||0).toLocaleString('pt-BR') + '</td>' +
            '<td class="bk-surv">' + (item.shaB*100).toFixed(1)+'%</td>' +
            '<td class="' + dCls(item.neto) + '">' + fDelta(item.neto) + '</td>' +
            '<td class="' + dCls(item.mix)  + '">' + fDelta(item.mix)  + '</td>' +
            '<td class="bk-impact-cell ' + iCls(item.impact) + '">' + fDelta(item.impact) + '</td>' +
            '</tr>';
    }});
    ...
    html += '<tr class="bk-total">' +
        '<td class="bk-name col-name">Total</td>' +
        '<td>' + fNPS(npsA_tot) + '</td>' +
        '<td>' + fNPS(npsB_tot) + '</td>' +
        '<td class="' + dCls(totDelta) + '">' + fDelta(totDelta!==null?Math.round(totDelta*100)/100:null) + '</td>' +
        '<td class="bk-surv">' + totB.s.toLocaleString('pt-BR') + '</td>' +
        '<td class="bk-surv">100%</td>' +
        '<td class="' + dCls(totNeto) + '">' + fDelta(totNeto) + '</td>' +
        '<td class="' + dCls(totMix)  + '">' + fDelta(totMix)  + '</td>' +
        '<td class="bk-impact-cell ' + iCls(totImpact) + '">' + fDelta(totImpact) + '</td>' +
        '</tr>';
