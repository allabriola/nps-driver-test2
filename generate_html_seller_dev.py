...
def _diagnostic_bullets(grp, bd_curr, bd_prev, nps_curr, nps_prev, lbl_curr, lbl_prev, is_vig=False):
    ...
    s_tot2 = sum(v["s"] for v in p2.values()) or 1
    proc_movers = []
...
def _analyze_trx_group(trx_dict):
    ...
    CONTACT = {
        ...
        "cobrança e faturamento":   ["cobrança","fatura","cobrado","certificado","faturador",
                                     "nota fiscal","nf-e","emissao","emitir","imposto",
                                     "cnpj","fiscal","tributar","boleto","tarifa","comiss",
                                     "ads","campanha"],
        "mediação e disputa":       ["mediação","mediacao","reclamo","disputa","divergência","inconformidade"],
        "reputação e conta":        ["reputação","suspens","minha conta","da conta","na conta",
                                     "minha reputação","conta foi","conta está","conta esta",
                                     "restrit","banid","penaliz","badge"],
...
def _bd_table(items_m1, items_m2, max_rows=6, weekly=False, lbl1="NPS", lbl2="Ant",
    ...
    rows = ""
    s_tot_m2_all = sum(v.get("s",0) for v in items_m2.values()) or 1
...
def _bd_seniority(sr_m1, sr_m2, weekly=False, lbl1="NPS", lbl2="Ant",
    ...
    s_tot_m2_all = sum(v.get("s",0) for v in (sr_m2 or {}).values()) or 1
