...
JOIN (
  ...
  WHERE DATE_ID = (SELECT MAX(DATE_ID) FROM `meli-bi-data.WHOWNER.BT_CX_KM_TRAINING_STATUS`)
    AND USER_TEAM_NAME = 'BR_ME_Sellers_Longtail'
) km ON T.USER_LDAP = km.USER_LDAP
...
SELECT
  TR.CONVERSATION_ID,
  SUBSTR(TR.COPILOT_TRANSCRIPT_OBFUSCATED, 1, 3000) AS copilot_transcript,
...
JOIN (
  ...
  WHERE DATE_ID = (SELECT MAX(DATE_ID) FROM `meli-bi-data.WHOWNER.BT_CX_KM_TRAINING_STATUS`)
    AND USER_TEAM_NAME = 'BR_ME_Sellers_Longtail'
) km ON M.USER_LDAP = km.USER_LDAP
LEFT JOIN `meli-bi-data.WHOWNER.LK_CX_PROCESS_ADM` P
...
WHERE TR.CONV_CREATED_DATE >= DATE_SUB(CURRENT_DATE(), INTERVAL 14 DAY)
  ...
  AND TR.COPILOT_TRANSCRIPT_OBFUSCATED IS NOT NULL
  AND LENGTH(TR.COPILOT_TRANSCRIPT_OBFUSCATED) > 100
ORDER BY RAND()
LIMIT 800
"""
