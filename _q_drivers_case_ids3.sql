-- Case IDs via BT_CX_CASE_INTERACTION com filtro simples por processo
SELECT
  og.CDU,
...
WHERE i.SIT_SITE_ID = 'MLB'
  ...
  AND og.OUTGOING_DATE >= '2026-04-01'
QUALIFY ROW_NUMBER() OVER (PARTITION BY og.CDU ORDER BY i.CI_CREATED_DATE DESC) <= 60
