...
ADOPT_TS = load_json("_copilot_adoption_ts.json", {"daily": []})
...
# Chips de mês (multi-seleção) — a partir dos meses presentes na série diária
...
<div class="container">
  ...
  <div id="tab-geral" class="tab-pane active">
    ...
    <!-- Linha 1: Evolução da Adoção | Distribuição & NPS -->
    <div class="ch-grid">
      <div class="cc">
    ...
    <div class="section-lbl" style="display:flex;justify-content:space-between;align-items:center;gap:12px">
      ...
      <button class="csv-btn" style="background:#188038" onclick="exportRepsXLSX()">&#11015; Exportar Excel</button>
    </div>
...
// Converte "100.0%" / "687s" / "3.5" em número quando possível (melhor no Excel/Sheets)
function _cellVal(txt) {{
...
function exportRepsXLSX() {{
  const table = document.getElementById('tbl-geral');
  if (!table) return;
  if (typeof XLSX === 'undefined') {{ alert('Biblioteca de Excel ainda carregando — tente novamente em 1s.'); return; }}
  const aoa = [];
  aoa.push([...table.querySelectorAll('thead th')].map(th => th.textContent.trim()));
  table.querySelectorAll('tbody tr').forEach(tr => {{
    if (tr.classList.contains('hidden')) return;
    aoa.push([...tr.querySelectorAll('td')].map(td => _cellVal(td.textContent)));
  }});
  const ws = XLSX.utils.aoa_to_sheet(aoa);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Reps');
  const hoje = new Date().toISOString().slice(0,10);
  XLSX.writeFile(wb, 'copiloto_reps_' + hoje + '.xlsx');
}}
</script>
</body>
</html>"""
